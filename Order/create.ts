...
export async function create(connection: cardfunc.Connection, order: payfunc.Order.Creatable, auto3d?: boolean): Promise<authly.Token | gracely.Error> {
	let result: authly.Token | gracely.Error
	const response = await post(connection, order)
	if (auto3d && payfunc.Payment.Card.Creatable.is(order.payment) && order.payment.card && cardfunc.Pares.missing(response)) {
	...
	export const command: paramly.Command<cardfunc.Connection> = {
		...
		execute: async (connection, argument, flags) => {
			...
			if (payfunc.Payment.Type.is(type))
				switch (type) {
					case "card":
						payment = await cardfunc.Authorization.verify(argument[1]) ? { type, reference: argument[1] } : { type, card: argument[1] }
						break
				}
			const result = connection && payment &&
